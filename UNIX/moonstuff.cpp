...
long cal2JH(int YEAR,int MONTH,int DAY,int HOUR)
{
  int I,J,K,JD;
  ...
  return JD*24+HOUR-12;
}


calendar JDtoCalendar(MYDOUBLE jd)
{
  int z;
  MYDOUBLE f;
  int alp,a,b,c,d,e;
  ...
  long jh2=cal2JH(cal.yy,cal.mm,cal.dd,cal.hr);
...
MYDOUBLE calendarToJD(calendar cal)
{	
  MYDOUBLE JDE;
  int a,b;
  MYDOUBLE d=(cal.dd+cal.hr/24);

  printf(" PJL doesn't think this works !!!!! \n");
...
void calcNewMoonPhase(const calendar cur, calendar &prev,calendar &next)
{
  ...
  if(cur.dd<=6)
    k=floor((cur.yy+((MYDOUBLE)(cur.mm-1)/12)-2000)*12.3685);
  else
    k=floor((cur.yy+((MYDOUBLE)(cur.mm)/12)-2000)*12.3685);
...
void calcFullMoonPhase(calendar cur,calendar &prev,calendar &next)
{
  ...
  if(cur.dd<=21)
    k=int((cur.yy+((MYDOUBLE)(cur.mm-1)/12)-2000)*12.3685)+0.5;
  else
    k=int((cur.yy+((MYDOUBLE)(cur.mm)/12)-2000)*12.3685)+0.5;
...
calendar  calcNewMoonPhaseK(MYDOUBLE k)
{
  ...
  MYDOUBLE dapp=-(0.40720*sin(md))+(.17241*e*sin(m))+(.01608*sin(2*md))+(.01039*sin(2*f))+(.00739*e*sin(md-m))
    -(.00514*e*sin(md+m))+(.00208*pow(e,2)*sin(2*m))-(.00111*sin(md-2*f))-(.00057*sin(md+2*f))
    +(.00056*e*sin(2*md+m))-(.00042*sin(3*m))+(.00042*e*sin(m+2*f))+(.00038*e*sin(m-2*f))
    -(.00024*e*sin(2*md-m))-(.00017*om)-(.00007*sin(md+2*m))+(.00004*sin(2*md-2*f))+(.00004*sin(3*m))
    +(.00003*sin(md+m-2*f))+(.00003*sin(2*md+2*f))-(.00003*sin(md+m+2*f))+(.00003*sin(md-m+2*f))
...
MYDOUBLE cal2Kish(calendar cal)
{
  return (cal.yy+((MYDOUBLE)(cal.mm-1)/12)-2000)*12.3685;
}
